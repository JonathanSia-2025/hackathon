...
if __name__ == "__main__":
    ...
    try:
        ...
        # Save predictions to fixed filename as requested (do not modify this file later)
        script_dir = os.path.dirname(os.path.abspath(__file__))
        ...
        # --- Build and save a separate anomalies/flags CSV (must NOT change the predictions CSV) ---
        # Risk thresholds applied to absolute values (user requested):
        def risk_label(v):
            a = abs(v)
            if a < 15:
                return 'LOW'
        ...
        # Create a separate DataFrame with tail id (if present) and per-factor risk labels
        risk_cols = []
        for col in TARGET_COLUMNS:
            risk_col = f"{col}_RISK"
            risk_cols.append(risk_col)

        # Start risk_df with TAIL_NUMBER if available, otherwise an index-based id
        if TAIL_NUMBER_COL in predictions_df.columns:
            risk_df = pd.DataFrame({TAIL_NUMBER_COL: predictions_df[TAIL_NUMBER_COL].values})
        else:
            risk_df = pd.DataFrame({'ROW_ID': predictions_df.index.values})

        # Populate risk columns based on predictions (do not alter predictions_df)
        for i, col in enumerate(TARGET_COLUMNS):
            risk_df[risk_cols[i]] = predictions_df[col].apply(risk_label)

        anomalies_path = os.path.join(out_dir, 'flag anomalies.csv')
        risk_df.to_csv(anomalies_path, index=False)
        print(f"Also saved anomalies CSV to: {anomalies_path}")

        # Print the anomalies DataFrame to terminal immediately after predictions
        print("\n--- Flag Anomalies (per-factor risk) ---")
        print(risk_df)
